...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            for (int i = 0; i < 40; ++i) {
                String inputsDirectoryPath = "C:\\Users\\asus\\Desktop\\projects\\Discrete Math final project template\\Q2\\TestGenerator\\inputs\\";
                FileWriter myWriter = new FileWriter(inputsDirectoryPath + "In_" + (i + 1) + ".txt");

                //generate airports number
                int airportsNumber = (int)(Math.random() * 10) + (i * 40);
                myWriter.write(airportsNumber + " ");
                System.out.println(airportsNumber);

                //generate edges number
                int edgesNumber = (int)(Math.random() * Math.min((airportsNumber * (airportsNumber - 1) / 2), 100000));
